...
const API_BASE_URL = 'https://eps.staging.fotball.no';
...
// Fix: Define props interface for the Login component for type safety.
      ...
      <form onSubmit={handleSubmit}>
        <div className="form-group">
          ...
          <input
            ...
            id="username"
            value={username}
            // Fix: Add type for the event object in the onChange handler.
...
// Fix: Define props interface for the Tracker component for type safety.
interface TrackerProps {
    token: string;
    onLogout: () => void;
}

function Tracker({ token, onLogout }: TrackerProps) {
  // State
  ...
  const searchForClubs = useCallback(async () => {
    ...
    try {
      ...
      // Use the new public endpoint for searching clubs
      const response = await fetch(`https://www.fotball.no/TournamentSearchPage/SearchClubs?term=${clubQuery}`);
      if (!response.ok) throw new Error('Failed to fetch clubs.');
      const rawData: { Id: number; Name: string }[] = await response.json();

      // Map the response to the app's Club interface
  ...
  const handleFetchPlayerUsage = async () => {
    ...
    try {
        ...
        // 1. Get match lists for each team (from cache or API)
        ...
        // 2. Get match details (from cache or API)
...
function App() {
  ...
  const handleLogin = (username: string, password: string) => {
    ...
    setTimeout(() => {
      if (username && password) {
        setToken('dummy-token'); // In a real app, this would be the token from the API
      } else {
  ...
  return (
    <div className="container">
      {token ? (
        <Tracker token={token} onLogout={handleLogout} />
      ) : (
